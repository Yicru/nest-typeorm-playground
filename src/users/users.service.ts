...
export class UsersService {
  ...
  private static createPasswordDigest(password: string) {
    return crypto
      .createHash('sha256')
  ...
  async register(userData: Partial<User>): Promise<void> {
    ...
    await this.userRepository.insert({
      ...userData,
      password: UsersService.createPasswordDigest(userData.password),
    });
    return;
  }
  ...
  async loginUser(screenName: string, password: string) {
    return await this.userRepository.findOne({
      where: {
        screenName,
        password: UsersService.createPasswordDigest(password),
      },
    });
  }
}
